...
reactor::reactor(int port, callback cb, int nr_workerthreads, int timeout_millisec, int nr_iothreads)
...
void reactor::start() {
    ...
    while ( true ) {
        ...
        if (items[0].revents & ZMQ_POLLIN) {
            liasion_.recv_from_req();
        }
        if (items[1].revents & ZMQ_POLLIN) {
            //todo: recvfrom router socket, send back to addr

//            std::string addr, content;
